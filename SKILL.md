...
### Step 0 — 验证论文内容（必须，硬性闸门！）

**在生成任何内容前，必须先读取 PDF 并验证论文主题。**

这一步至关重要——如果论文主题与预期不符，后续所有内容都会错误。

#### 强制确认规则

Step 0 是不可跳过的硬性确认闸门，必须严格执行：

- **必须等待用户明确回复确认**，例如“确认”“正确”“继续”“没问题”等同等含义表达。
- **不得将用户沉默、未反对、未及时回复、上下文中看似合理，视为确认。**
- **在收到明确确认前，禁止创建课程目录、生成 HTML 模块、生成 Markdown、生成 PPTX、运行构建脚本或写入任何课程输出文件。**
- 如果用户指出主题、标题、作者、摘要不符，必须先重新读取/核对论文并再次发起确认。

读取 PDF 后，必须提取并确认：
- **标题**：论文原标题
- **作者/机构**：论文作者和所属机构
- **Abstract**：摘要内容
- **关键词**：论文的关键词。如论文未提供可省略。
...
**只有在用户明确确认后，才继续生成内容。未收到明确确认时必须停止在 Step 0。**
...
- 主要研究发现
- 研究贡献
以上全部内容应且仅应来源于论文的摘要和引言部分，禁止使用论文其他部分内容。如论文未明确区分引言部分，应使用论文的第一章视为引言。

**Module 2: 文献综述**
该模块总结核心研究问题相关领域脉络。如果论文中没有明确的文献综述部分，则使用引言或第一章内容进行生成。有些论文不会以“文献综述”命名章节，其他可能的章节或者子章节标题有“conceptual framework”，“backround”，“institutional background”, "theory", "theoretical background"等，需根据论文内容判断并提取相关内容生成文献综述模块。

**Module 3: 假设发展**

**Module 3: 方法对比**
如果论文中没有明确的主流方法对比部分，则使用相关工作部分内容进行生成，重点突出与本文方法的区别和联系。
